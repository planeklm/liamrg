...
import {
  ...
  onMount,
  set_attributes
} from "./chunk-2V2F4FLU.js";
import {
  append,
  comment,
  template
} from "./chunk-KWDYW75E.js";
import {
  FILENAME,
  HMR,
  child,
...
function mergeIconData(parent, child2) {
  ...
  for (const key in defaultExtendedIconProps) {
    ...
    } else if (key in child2) {
      result[key] = child2[key];
...
var root_2 = add_locations(template(`<svg><!></svg>`), Icon[FILENAME], [[112, 2]]);
var root_3 = add_locations(template(`<span></span>`), Icon[FILENAME], [[116, 2]]);
function Icon($$anchor, $$props) {
  ...
  let mounted = mutable_state(false);
  let counter2 = mutable_state(0);
  let data = mutable_state();
  const onLoad = (icon) => {
    if (strict_equals(typeof $$sanitized_props.onLoad, "function")) {
  ...
  onDestroy(() => {
    mutate(state, get(state).destroyed = true);
    if (get(state).loading) {
      get(state).loading.abort();
      mutate(state, get(state).loading = null);
    }
  });
  legacy_pre_effect(
    () => (get(counter2), checkIconState, deep_read_state($$sanitized_props), get(state), get(mounted), get(data), generateIcon),
    () => {
      get(counter2);
      const iconData = checkIconState($$sanitized_props.icon, get(state), get(mounted), loaded, onLoad);
      set(data, iconData ? generateIcon(iconData.data, $$sanitized_props) : null);
      if (get(data) && iconData.classes) {
        mutate(data, get(data).attributes["class"] = (strict_equals(typeof $$sanitized_props["class"], "string") ? $$sanitized_props["class"] + " " : "") + iconData.classes.join(" "));
      }
    }
  );
  legacy_pre_effect_reset();
  ...
  if_block(node, () => get(data), ($$anchor2) => {
    ...
    if_block(
      ...
      ($$anchor3) => {
        ...
        html(node_2, () => get(data).body, true, false);
        reset(svg);
        template_effect(() => attributes = set_attributes(svg, attributes, { ...get(data).attributes }, void 0, true));
        append($$anchor3, svg);
      },
      ($$anchor3) => {
        var span = root_3();
        let attributes_1;
        template_effect(() => attributes_1 = set_attributes(span, attributes_1, { ...get(data).attributes }));
        append($$anchor3, span);
      }
    );
